...
for aSample in sampleNumbers:
  ...
  dst = os.path.join('results_stoch', 'sc' + str(ParametersProjection.getScenario()),'2030_' + str(aSample) + '.map') # Renan: I added the "sc" folder here
  shutil.copy2(src, dst)
  # also 2020, can be removed if required
  src = os.path.join(str(aSample), 'landUse0.015')
  dst = os.path.join('results_stoch', 'sc' + str(ParametersProjection.getScenario()), '2020_' + str(aSample) + '.map') # Renan: I added the "sc" folder here
  shutil.copy2(src, dst)
