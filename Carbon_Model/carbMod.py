...
def getVariance(dictio_stockPerMCr, sc):
    """Get the variance of ... """  
    var = np.var(totStockPerMCr).astype(np.float64)
    print var
    return var
...
def plotSensitivityAnalysis():
    ...
    d_var = {k:"" for k in components}
    for i in components:
        #d_tCperMCr = {sc:np.load(os.path.join(outDir, "det_runs", "sc{}_{}_tCperMCr.npy".format(sc, i))) for sc in scRange}
        #d_tCperMCr = openPickleFile(os.path.join(parameters.tempDir), 'd_{}_tCperMCr.pickle'.format(i))
        diffperPair = getDiffStockPerPairScenarios(d_tCperMCr)
        #print i, "\n", diffperPair
        var_array = {k:{} for k in diffperPair.keys()}
        for sc in var_array.keys():
            var = getVariance(diffperPair[sc], sc)
            var_array[sc] = var
        d_var[i] = var_array
    
    print d_var
    variance_data = pd.DataFrame.from_dict(d_var, orient='columns', dtype=np.float64)
    print variance_data
    
    varFract = (variance_data.div(variance_data['stcAll'], axis=0))*100
    varFract['tot'] = varFract['stcBC'] + varFract['stcSOC'] + varFract['stcLU']
    ...
    uncBC = plt.bar(bars, varFract.stcBC, color=colors[0], width=barWidth)
    uncSOC = plt.bar(bars, varFract.stcSOC, bottom=varFract.stcBC, color=colors[1], width=barWidth)
    uncLU = plt.bar(bars, varFract.stcLU, bottom=varFract.stcBC+varFract.stcSOC, color=colors[2], width=barWidth)
    uncOther = plt.bar(bars, varFract['diff'], bottom=varFract['tot'], color=colors[3], width=barWidth)
    ...
    plt.legend((uncBC[0], uncSOC[0], uncLU[0], uncOther[0]), ('Biomass', 'SOC', 'LUC', 'Model interactions'), bbox_to_anchor=(0.78,-0.12), fontsize=10, ncol=4)
    plt.grid(which="major", axis="y", color='0.7', linewidth=0.3)
    plt.subplots_adjust(bottom=0.2)
    if savePlots == 1:
        plt.savefig(os.path.join(resultsDir, 'sens_analysis_diff_all'), dpi=700)
    if showPlots == 1:
        plt.show()    
...
def getStockPerLUtype(arrayStocks, arrayLU_ravel):
    """Return a numpy.ndarray with the total carbon stocks (in tonne C/ha) per LU type. The array index represents the LU types.   
    Arguments: arrayStocks = array representing the SOC or BC stocks in the study area; arrayLU = array representing the arrayStocks-related LU map"""
    # Ravelling arrayStocks to use it into a pandas dataframe 
    arrayStocks = arrayStocks.ravel()
    stockPerCell = pd.DataFrame(data=arrayStocks, index=arrayLU_ravel) # dataframe with the amount of stocks for each cell
    stockPerLu = stockPerCell.groupby(level=0).sum() # dataframe resulting in total carbon stocks (column) per LU type (row)  
    return stockPerLu.values #return the values as numpy.ndarrays

def zerosArray(referenceRaster):
    """ Create an array filled with zero, necessary to compute mean and std of cell-based carbon stocks. The shape size is based on the number of columns and rows of a raster file. Obs: use numpy.zeros and do not use numpy.empty (it does not work for the computations used in this script). Arguments: referenceRaster = reference raster of the model."""
...
def arrayToMap(npArray, outRaster, saveToPath):
    """Convert an array to a raster map and save to disk (tiff format). Arguments: origRaster = raster used to get geoparameters and 'no data' cells; npArray = array to be converted; outRaster = string with the filename representing the raster map; saveToPath = path to save the raster map"""
    ...
    npArray[~nvMsk.mask] = -9999  # Updating npArray with the null value of the MskArr
    
    # Creating new raster, then adding geoparameters
    driver = gdal.GetDriverByName('GTiff')
    outRaster = driver.Create(os.path.join(saveToPath, outRaster + ".tiff"), cols, rows, 1, gdal.GDT_Float64)
    outRaster.SetGeoTransform((originX, pixelWidth, 0, originY, 0, pixelHeight))
    outband = outRaster.GetRasterBand(1)
    outband.WriteArray(npArray)
...
def cellStocks_analysis(csType, luRaster, sc, inPath, saveTo):
    """XXXXXX"""
    lu = rasterToArray(rLU)   
    luMask = np.ma.masked_where(((lu <= 2) | (lu == 10)), lu)    
    npFiles = [npFile for npFile in sorted(glob.iglob(os.path.join(inPath, "arrays_sc{}mc*".format(sc))))]
    # Computing total cell-based mean and per LU type 
    cs_accum = np.zeros([885, 854], dtype=np.float64) 
    csPerLU_accum = np.zeros([12, 1], dtype=np.float64)
    for mcRun in npFiles:
        ...
        csPerLU_accum = csPerLU + csPerLU_accum
        #print csPerLU_accum
        #print 'Processing', mcRun.split("_")[4], "\n", np.unique(cs_accum)
        #with open(os.path.join(out Dir, "checkNan.txt"), 'a') as f:
            #if np.isnan(cs_accum).any() == True:
                #f.write('\n', "cs_accum", cs.dtype, csType, sc, mcRun.split("_")[4])	
    csMean_cell = cs_accum/len(npFiles) # OK! (It has 0 values)
    csMean_cell[luMask.mask] = 0 
    csMean_cell_PerLU = (csPerLU_accum/len(npFiles))
    # Computing cell-based STD
    cs_sqAccum = np.zeros([885, 854], dtype=np.float64)
    csPerLU_sqAccum = np.zeros([12, 1], dtype=np.float64)
    for mcRun in npFiles:
        npData = np.load(mcRun)
        cs = npData[csType]
...
def diffInit_cellStocks_analysis(scEth, csType, luRaster, inPath, saveTo):
    "XXXXX"
    lu = rasterToArray(luRaster)
    luInit = rasterToArray(luScenarios[0])  
    luMask = np.ma.masked_where(((lu <= 2) | (lu == 10)), lu)    
    npFiles = [npFile for npFile in sorted(glob.iglob(os.path.join(inPath, "arrays_sc{}mc*".format(scEth))))]
    npFiles_init = [npFile for npFile in sorted(glob.iglob(os.path.join(inPath, "arrays_sc0mc*")))]
    # Computing MEAN cell-based diff
    acc_diff = np.zeros([885, 854], dtype=np.float64) 
    acc_diffPerLU = np.zeros([12, 1], dtype=np.float64)
    for mcRun, mcRun_init in zip(npFiles, npFiles_init):
        cs = np.load(mcRun)[csType]
        csInit = np.load(mcRun_init)[csType]
        diff = cs-csInit
        acc_diff = diff + acc_diff
        # Computation Per LU
        csPerLU = getStockPerLUtype(cs, lu)        
        csInitPerLU = getStockPerLUtype(csInit, luInit)                
        diffPerLU = (csPerLU - csInitPerLU).astype(np.float64)
        acc_diffPerLU = diffPerLU + acc_diffPerLU
        print scEth, '- mean -', mcRun_eth.split("_")[5], mcRun_noEth.split("_")[5], np.isnan(acc_diff).any()
    mDiff = acc_diff/len(npFiles)
    mDiffperLU = acc_diffPerLU/len(npFiles)
    # Computing STD cell-based diff
    acc_sqDiff = np.zeros([885, 854], dtype=np.float64) 
    acc_sqDiffPerLU = np.zeros([12, 1], dtype=np.float64)
    for mcRun, mcRun_init in zip(npFiles, npFiles_init):
        ...
        sqDiffPerLU = (diffPerLU - mDiffperLU)**2
        acc_sqDiffPerLU = sqDiffPerLU + acc_sqDiffPerLU    
    stdDiff = np.sqrt(acc_sqDiff/len(npFiles)) 
    stdDiff = np.asarray(stdDiff)
    stdDiff[luMask.mask] = 0    
    stdDiffperLU = np.sqrt(acc_sqDiffPerLU/len(npFiles))
    #print stdDiffperLU    
    np.save(os.path.join(saveTo, "dsc{}Xinit_mTC_cell".format(scEth)), mDiff.astype(np.float32))
    np.save(os.path.join(saveTo, "dSc{}Xinit_stdTC_Cell".format(scEth)), stdDiff.astype(np.float32))
    #print 'SC{} vs SC{}\t|\t ... {} cell-based mean diffs are now saved'.format(csType, scEth, scEth-1)
    if os.path.exists(os.path.join(outDir, "csDiff12_{}perLU_mean.txt".format(csType))) == True and os.path.exists(os.path.join(outDir, "csDiff12_{}perLU_std.txt".format(csType))) == True:
        with open((os.path.join(outDir, "csDiff12_{}perLU_mean.txt".format(csType))),"a+") as mean:
            mean.write('sc{};'.format(sc))           
            np.savetxt(mean,mDiffperLU.T, delimiter=';')    
        with open((os.path.join(outDir, "csDiff12_{}perLU_std.txt".format(csType))),"a+") as std:
            std.write('sc{};'.format(sc))           
            np.savetxt(std,stdDiffperLU.T, delimiter=';')      
    else:
        with open((os.path.join(outDir, "csDiff12_{}perLU_mean.txt".format(csType))),"w+") as mean:
            mean.write("scen;"+";".join([str(i) for i in  range(len(mDiffperLU))])+'\nsc{};'.format(sc))
            np.savetxt(mean,mDiffperLU.T, delimiter=';')    
        with open((os.path.join(outDir, "csDiff12_{}perLU_std.txt".format(csType))),"w+") as std:
            std.write("scen;"+";".join([str(i) for i in  range(len(stdDiffperLU))])+'\nsc{};'.format(sc))
            np.savetxt(std,stdDiffperLU.T, delimiter=';')      
    #return mDiff, stdDiff    
...
for sc, rLU in enumerate(luScenarios):
    starttime = time.time()
    print ("SC{}\t|\tGetting cell-based carbon stocks per MCr (tonne C/ha)...\n".format(sc))
    mSOC_cell = cellStocks_analysis('SOC', rLU, sc, inPath=os.path.join(tempDir, 'npz_arrays'), saveTo=tempDir)
