...
# then adding the deterministic initial LU map (k = 0, v = LU map path)
scenariosDict = carbParams.getScenariosPaths(opj('PLUC_model/Results'), 'sc*')
    ...
    for i in mcRange:
        ...
        with open(inpTxt, 'r') as inp:
            ...
            for row in inp:
                ...
                while SOCr <= 0:
                    print (
                        'Neg/zero value identified for SOCr...Getting positive value ...')
                    SOCr = np.random.normal(SOCr_mean, SOCr_std)
            ...
            for row in inp:
                ...
                # Computing SOCf for LU types w/ carbon stocks
                    ...
                    while (fLU <= 0 or fMG <= 0 or fI <= 0):
                        ...
                        fI = np.random.normal(fI_mean, fI_std)
                        print (
                            'Neg. value identified for SOC factor(s). Getting positive value(s)...')
                    SOCf = fLU * fMG * fI
    ...
    for i in mcRange:
        ...
        with open(inpTxt, 'r') as inp:
            ...
            for row in inp:
                ...
                while r2s < 0:
                    print (
                        'Neg. value identified for r2s. Getting positive value by truncation ...')
                    if r2s_std != 0:  # scipy.stats does not allow computation of std = 0
                        a, b = (r2s_min - r2s_mean) / r2s_std, (r2s_max -
                                                                r2s_mean) / r2s_std  # setting truncation
                ...
                bgb = agb * r2s
                BCS = (agb + bgb) * cf
                # if clause necessary to compute BCS for CROPs and ABANDONNED LAND (IPCC doesn't give AGB, R2S and CF for those LU types)
                if BCS == 0:
...
def reclassArray(npRaster, txtFile):
    ...
    cvrm = np.loadtxt(txtFile, dtype=np.int32,
                      skiprows=1)  # 'Code-value' reference matrix: convert the .txt into array
    code, value = cvrm.T  # Transposed array representing code and value
    # Lookup table with all the codes of npRaster. '+1' is necessary to encompass all the codes
    lut = np.arange(npRaster.max() + 1)
    # Assign a value for the lookup table based on the associated code
    lut[code] = value
    # Reclassify the npRaster filled with codes according to the values of the lookup tables
...
# <============== Function to get the LUmap depending on the scenario nr LUC component
...
def getLUmapMCrange(nrLUmap, mcRuns_LUmap, mcRun_accumulated):
    """Return the Monte Carlo samples Range for each LUC map. It is used when LUC_comp == 1 (stochastic)). Arguments: LUmapsDict = dictionary with the LUC maps to be used and related MC runs based on Particle Filter Mapping (see carbParams.getStochasticLUmap function); nrLUmap = code of the LU map; mcRuns_LUmap: total mcRuns of a given land use map;  mcRun_accumulated = the current accumulated MCrun of the for loop (since this function is run in a for loop)"""
...
# <============== Function to get the Nr of Cells per LU type
def getNrCellsPerLUtype(arrayLU):
...
def getTotalBCstock(climLu, LUmask, BC_comp):
    """Returns the total biomass carbon stocks for a given land use map (bc == array with total stock per cell, tBC == sum of array). The outputs depend whether the 'BC_opjcomp' variable is set to run deterministically or stochastically. Arguments: climLU = array of climate + lu maps combination (for SOC factor); LUmask: it is the output of the maskNoCarbonCells() function. BC_comp = setup of the BC component (see carbParams.configModelComponents())"""
    # Setting the path of txt files (depending if stochastic or not)
    ...
    if BC_comp == 0:  # deterministic for sensitivity analysis
        txtFile_bcs = opj('bcs_sensAnalysis.txt')
...
def IPCCdet_getDiffStockPerPairScenarios():
    ...
    for sc, scPath in scenariosDict.iteritems():
        ...
        if sc > 0:
            LUmapPath = carbParams.getDeterministicLUmap(
                scPath, particleFilter_mapping)
            print (LUmapPath)
...
def getStats_OverallCarbonStock(statsType, outFile=""):
    ...
    if statsType == 'absolute':
        ...
        raise Exception(
            "The 'statsType' argument is wrong. The user must set it as 'absolute' OR 'diff_ethanol' OR 'diff_ethanol_det'")
    ...
    for i, (stockType, dictio) in enumerate(zip(stockTypesList, stockDictiosList)):
        ...
        for sc, arr in dictio.iteritems():  # arr = array containing overall totStocks of each MCrun
            ...
            toSave = {'{}mean'.format(stockType): round(mean, 2), '{}median'.format(stockType): round(median, 2), '{}std'.format(stockType): round(std, 2), '{}var'.format(stockType): round(var, 2), '{}percLow'.format(
                stockType): round(percLow, 2), '{}percHigh'.format(stockType): round(percHigh, 2), '{}uncLow'.format(stockType): round(uncLow, 2), '{}uncHigh'.format(stockType): round(uncHigh, 2)}
            ...
            if i == 0:
                ...
                for k, v in meanToReturn.iteritems():
                    meanDict[sc][k] = v
            print (
                'SC{} -\t|\tMean: {:.3f} ({:.2f}% , {:.2f}%)'.format(sc, mean, uncLow, uncHigh))
...
def getEthanolDiffProduction(filename):
    ...
    data = pd.read_csv(filename, sep=",", header=0, usecols=[
                       'scenario', 'ethProd_mln_l'], index_col='scenario', skiprows=[1], dtype=np.int32)
    ...
    for stockType, dictio in zip(stockTypesList, stockDictiosList):
        for (k1, array_carbonStock), (k2, deltaEth) in zip(dictio.iteritems(), ethDiff.iteritems()):
            if k1 == k2:  # k1 and 2 are both scenario codes
                ...
                # converting cStocks (tonne/ha) in total tonnes
                array_hectares = array_carbonStock * carbParams.getConversionUnit()
                # converting cStocks to emission Factor
...
def plotBoxplot():  # OBS: I still couldn't figure out how to use a marker instead of a line in the "usermedians" argument of boxplot, that's why legend and boxplot don't match for "deterministic"
    ...
    if plotFigures == 1:
        ...
        socBoxes = plt.boxplot(socGHGe.values(), positions=np.array(xrange(len(socGHGe.values()))) * 2 + 0.4,
                               sym='', usermedians=socGHGe_det.values(), meanline=True, showmeans=True, patch_artist=True, widths=0.6)
        bcBoxes = plt.boxplot(bcGHGe.values(), positions=np.array(xrange(len(bcGHGe.values()))) * 2 - 0.4,
                              sym='', usermedians=bcGHGe_det.values(), meanline=True, showmeans=True, patch_artist=True, widths=0.6)
        setupBoxplot(bcBoxes, colors[1])
        setupBoxplot(socBoxes, colors[0])
        ymin, ymax = (int(np.max(np.stack((socGHGe.values(), bcGHGe.values())))
                          * -1), int(np.max(np.stack((socGHGe.values(), bcGHGe.values())))))
        plt.yticks(np.arange(ymin, ymax + 1, step=5))
        plt.xticks(np.arange(0, len(socGHGe.values()) * 2, 2),
...
def plotSensitivityAnalysis():
    ...
    if runSensAnalysis == 1:
        ...
        barOther = plt.bar(
            bars, varFract['diff'], bottom=varFract['tot'], color=colors[3], width=barWidth)
        # Plot setup
        ...
        plt.legend((barBC[0], barSOC[0], barLUC[0], barOther[0]), ('Biomass', 'SOC', 'LUC',
                                                                   'Model interactions'), bbox_to_anchor=(0.78, -0.12), fontsize=10, ncol=4)
...
#-----------------     PROCESSING OVERALL CARBON STOCKS     -------------------#
...
if getOverallCarbonStock == 1:
    ...
    # <=============== Here is the dictionary which I compute nr of Cells per LU
    d_CellsPerLU = {sc: '' for sc in scenariosDict.keys()}
    # Running stochastic
    for sc, scPath in scenariosDict.iteritems():
        print ('\nSC{} - computing carbon stocks (tonne C/ha)\t...\tStart time: {}'.format(sc,
        ...
        if LUC_comp == 0 or sc == 0:
            ...
            print ('\n\tLUmap (deterministic):\tsc{}/{}\n'.format(sc,
                                                                  LUmapPath.split("/")[-1]))
            # <=============== # Here is where I call the function to compute Nr of Cells, according to the LU map that is got by getLUmap()
            d_CellsPerLU[sc] = getNrCellsPerLUtype(LUmap)
        ...
        if LUC_comp == 1 and sc > 0:
            # iterator to count nr of mcRuns (necessary to change the LU map according to particle filter
            mcRun_acc = 0
            ...
            for mapDescr in sorted(LUmapsDict.items()):
                ...
                print ('\n\tLUmap nr {} (stochastic): {}\t|\tsamples: {} (runs {}-{})\n'.format(
                    mapCode, LUmapPath.split("ts/")[-1], mapMCruns, min(mcRange_curr), max(mcRange_curr)))
                for sample in mcRange_curr:
                    ...
                    if saveArrays == 1 and modelRunType == 'Full stochastic':
                        # next: add the map array thing mentioned by Judith to reduce disk storage
                        np.savez_compressed(
                            opj(resultsDir, 'arr_cellBased', 'cb_sc{}mc{}'.format(sc, sample)), soc=soc, bc=bc)
                    print ('\tSC{} MC{}\t|\t{}\t|\ttSOC: {}\ttBCS: {}\ttCS: {} (ton C/ha)'.format(
                        sc, sample, modelRunType, tSOC, tBC, tC))
        if saveArrays == 1:
    ...
    if LUC_comp == 0:
        ...
        df.to_csv(opj(resultsDir, "d_CellsPerLU_det.txt"), sep=',')


if getOverallCarbonStock == 0:
    # Opening numpy arrays with total stocks of each MC sample
    d_tSOCperMCr = {sc: np.load(opj(resultsDir, 'arr_totalStocks', '{}_sc{}_tSOC.npy'.format(
        modelRunType, sc))) for sc in scenariosDict.keys()}
    d_tBCperMCr = {sc: np.load(opj(resultsDir, 'arr_totalStocks', '{}_sc{}_tBC.npy'.format(
        modelRunType, sc))) for sc in scenariosDict.keys()}
    d_tCperMCr = {sc: np.load(opj(resultsDir, 'arr_totalStocks', '{}_sc{}_tC.npy'.format(
        modelRunType, sc))) for sc in scenariosDict.keys()}
...
scStats = getStats_OverallCarbonStock(
    'absolute', '{}_absolute_Stats.csv'.format(modelRunType))
scDiffStats = getStats_OverallCarbonStock(
    'diff_ethanol', "{}_dEth_Stats.csv".format(modelRunType))
...
socGHGe, bcGHGe, tcGHGe = getEmissionsDueToEthanol(
    diff_tSOCeth, diff_tBCeth, diff_tCeth, ethDiff, '{}_GHGe.csv'.format(modelRunType))
...
#-------------------     OUTPUTS (BOXPLOT, SENSITIVITY ANALYSIS, DATA FOR STATISTICAL TEST      -------------------#

# Boxplot
plotBoxplot()

# For sensAnalysis, the model must run four times (see carbParams.configModelComponents()): "full stochastic", "stcSOC", "stcBC" and "stcAll".
...
print ("\nDone.\n\n---\t{0:.2f} sec ({1:.2f} min)\t---".format(
    float(time.time() - start), float((time.time() - start) / 60)))
...
def getCellMean(csType, sc, saveTo):
    """XXXXXX"""
    npzFiles = sorted(
        glob.glob(opj(resultsDir, 'arr_cellBased', 'cbStocks_sc{}mc*'.format(sc))))
    npzFilesDict = {
    ...
    if sc == 0:
        ...
        for sample, fl in npzFilesDict.iteritems():
            ...
            if np.isnan(cs_accum).any() == True:
                print ('SC{} - NaN found in cs_accum when summing {} with file {}'.format(
                    sc, csType, npzFilesDict[sample]))
    if sc > 0:
        # iterator to count nr of mcRuns (necessary to change the LU map according to particle filter
        mcRun_acc = 0
        LUmapsDict = carbParams.getStochasticLUmaps(
        ...
        np.save(opj(resultsDir, "sc{}_{}_mean".format(sc, csType)), csMean_cell)
        ...
        for mapDescr in sorted(LUmapsDict.items()):
            ...
            #np.save(opj(saveTo, "sc{}_m{}perCell".format(sc,csType)), csMean_cell.astype(np.float32))
            #np.save(opj(saveTo, "sc{}_std{}perCell".format(sc,csType)), csStd_cell.astype(np.float32))
            # Savin CS per LU
            # if os.path.exists(opj(outDir, "cs_{}perLU_mean.txt".format(csType))) == True and os.path.exists(opj(outDir, "cs_{}perLU_std.txt".format(csType))) == True:
                # with open((opj(outDir, "cs_{}perLU_mean.txt".format(csType))),"a+") as mean:
                    # mean.write('sc{};'.format(sc))
                    #np.savetxt(mean,csMean_cell_PerLU.T, delimiter=';')
                # with open((opj(outDir, "cs_{}perLU_std.txt".format(csType))),"a+") as std:
                    # std.write('sc{};'.format(sc))
                    #np.savetxt(std,csStd_cell_PerLU.T, delimiter=';')
            # else:
                # with open((opj(outDir, "cs_{}perLU_mean.txt".format(csType))),"w+") as mean:
                    #mean.write('MEAN stock per LU\nsc{};'.format(sc))
                    #np.savetxt(mean,csMean_cell_PerLU.T, delimiter=';')
                # with open((opj(outDir, "cs_{}perLU_std.txt".format(csType))),"w+") as std:
...
def diffEth_cellStocks_analysis(scEth, csType, luRaster, inPath, saveTo):
    ...
    for mcRun_eth, mcRun_noEth in zip(ethFiles, noEthFiles):
        ...
        csNoEth = np.load(mcRun_noEth)[csType]
        diff = csEth - csNoEth
        # mask to remove zero values when doing the computation of squares
    ...
    if os.path.exists(opj(outDir, "csDiff_{}perLU_mean.txt".format(csType))) == True and os.path.exists(opj(outDir, "csDiff_{}perLU_std.txt".format(csType))) == True:
        with open((opj(outDir, "csDiff_{}perLU_mean.txt".format(csType))), "a+") as mean:
            mean.write('sc{}Xsc{};'.format(sc, sc - 1))
            np.savetxt(mean, mDiffperLU.T, delimiter=';')
        with open((opj(outDir, "csDiff_{}perLU_std.txt".format(csType))), "a+") as std:
            std.write('sc{}Xsc{};'.format(sc, sc - 1))
            np.savetxt(std, stdDiffperLU.T, delimiter=';')
    else:
        with open((opj(outDir, "csDiff_{}perLU_mean.txt".format(csType))), "w+") as mean:
            mean.write(
                "scen;" + ";".join([str(i) for i in range(len(mDiffperLU))]) + '\nsc{}Xsc{};'.format(sc, sc - 1))
            np.savetxt(mean, mDiffperLU.T, delimiter=';')
        with open((opj(outDir, "csDiff_{}perLU_std.txt".format(csType))), "w+") as std:
            std.write(
                "scen;" + ";".join([str(i) for i in range(len(stdDiffperLU))]) + '\nsc{}Xsc{};'.format(sc, sc - 1))
            np.savetxt(std, stdDiffperLU.T, delimiter=';')
...
if getCellBasedCarbonStock == 1:
    LUC_comp = 1
    for sc, scPath in scenariosDict.iteritems():
        print ('\nSC{} - computing cell-based carbon stocks (tonne C/ha)\t...\tStart time: {}'.format(sc,
                                                                                                      time.asctime(time.localtime(time.time()))))
        mSOC_cell = cellStocks_analysis('soc', sc, saveTo=resultsDir)
        #mBC_cell = cellStocks_analysis('bc', rLU, sc, inPath=opj(tempDir, 'npz_arrays'), saveTo=tempDir)
        #mTC_cell = cellStocks_analysis('tc', rLU, sc, inPath=opj(tempDir, 'npz_arrays'), saveTo=tempDir)
        # print ("SC{}\t|\t Cell-based processing took {:.2f} min\n".format(sc, float(time.time()-starttime)/60)))

time_cs_cell = time.time()
print ("Finished. Execution time: {:.2f} min (process) / {:.2f} min (total)".format(
    float((time_cs_cell - time_cs) / 60), float((time_cs_cell - start) / 60)))
...
for data in dataLU:
    data_ha = pd.read_csv(opj(outDir, data), sep=";", header=0, index_col=0)
    data_ha = data_ha.loc[:, (data_ha != 0).any(axis=0)]
    data_GHG_20yrs = (data_ha * 2500 * 44 / 12.0) / 20
    data_GHG_20yrs.columns = ['Natural forest', 'Rangeland', 'Planted forest',
                              'Crops', 'Grass and shrubs', 'Sugar cane', 'Planted pasture', 'Abandoned']
    data_GHG_20yrs = data_GHG_20yrs.reindex(['Natural forest', 'Planted forest', 'Grass and shrubs',
                                             'Rangeland', 'Crops', 'Planted pasture', 'Sugar cane', 'Abandoned'], axis=1)
    # To plot considering division by increase in ethanol production...
    df_dMgt.set_index(data_ha.index, inplace=True)
    data_GHG_20yrs_ETH = (data_GHG_20yrs.div(
        df_dMgt['dEth_Gj'], axis=0)) * -1 * 1000
...
lu_type = ['Natural forest', 'Planted forest', 'Grass and shrubs',
           'Rangeland', 'Crops', 'Planted pasture', 'Sugar cane', 'Abandoned']
lu_colors = ['#385623', '#75b54a', '#2efe2e', '#ed7d31',
             '#cfaa13', '#843c0b', '#ff3787', '#6e6e6e']
scenarios = ['Reference\nscenario', 'High\nproductivity', r'2$^n$$^d$ generation' + '\nSugar cane',
             r'2$^n$$^d$ generation' + '\nEucalyptus', 'Conservation\npolicies', 'All\nmeasures']
...
plt.yticks(np.arange(round(np.min(mTC_lu + stdTC_lu)) - 2,
                     round(np.max(mTC_lu - stdTC_lu)) + 1, step=10))
...
plt.yticks(np.arange(round(np.min(mTC_lu + stdTC_lu)) - 2,
                     round(np.max(mTC_lu - stdTC_lu)) + 1, step=5))
...
for (i, mean), std in zip((enumerate(mTC_lu)), stdTC_lu):
    ...
    plt.bar(0.15 + lu_classes + i * gap, mean, width=gap, yerr=std, color=lu_colors[i % len(
        lu_colors)], edgecolor='#898989', linewidth=0.3, alpha=0.8, error_kw=errorLine)
...
if saveFigures == 1:
    plt.savefig(opj(outDir, 'barchart_LU_{}'.format(plt_name)), dpi=700)
plt.show()
